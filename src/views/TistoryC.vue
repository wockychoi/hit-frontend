...
export default {
  ...
  try {
    // 쿠키 생성 로직 (우선: this.tempCookie, 없으면 localStorage 조합)
    let cookies = "";

    const parseCookiesFromInput = (input) => {
      const s = input.trim();

      // 1) 시도: 정상 JSON 파싱 (배열 또는 객체)
      try {
        ...
        if (parsed && typeof parsed === "object") {
          // 객체가 {name:value,...} 아니면 {cookies:[{...},...]}
          if (Array.isArray(parsed.cookies)) {
            return parsed.cookies.map(c => `${c.name}=${c.value}`).join("; ");
          }
          // 일반 객체 -> key:value 쌍으로 변환
          return Object.entries(parsed).map(([k, v]) => `${k}=${v}`).join("; ");
        }
        return String(parsed);
      } catch (e) {
        // 2) 실패하면 간단한 정규식으로 name=value 추출 (single-quote, True 등 부정확 포맷에도 어느정도 대응)
    ...
    // 디버그 로그(필요하면)
    // console.log("X-Tistory-Cookie:", cookies);
